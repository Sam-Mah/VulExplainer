...
def load_vul_ground_truth(shuffle=True):
    """Load the ground truth from the ba2motif dataset.

    :param shuffle: Wether the data should be shuffled.
...
def load_dataset_ground_truth(_dataset, test_indices=None):
    ...
    if _dataset == "syn4":
        ...
        edge_index, labels = load_vul_ground_truth(shuffle=True)
        allnodes = [i for i in range(0, 100)]
        allnodes.extend([i for i in range(500, 600)])
        if test_indices is None:
            return (edge_index, labels), allnodes
